...
from harmoni_common_lib.constants import DetectorNameSpace, SensorNameSpace, ActionType, State
...
class TestW2L_Common(unittest.TestCase):
    ...
    def setUp(self):
        self.output_sub = rospy.Subscriber("/harmoni/detecting/stt/default", String, self._detecting_callback)
        ...
        self.client.setup_client(self.server, self._result_callback, 
                                 self._feedback_callback, wait=True)
        rospy.loginfo("TestW2L: Turning ON stt server")
        self.client.send_goal(action_goal=ActionType.ON, optional_data="Setup",
                              wait=False)
    ...
    def _detecting_callback(self, data):
        rospy.loginfo(f"Detecting: {data}")
        self.result = True

class TestW2L_Valid(TestW2L_Common):
    def test_IO(self):
        rospy.loginfo("TestW2L[TEST]: basic IO test to ensure data " +
            "('hello' audio) is received and responded to. Waiting for transcription...")
        while not rospy.is_shutdown() and self.feedback != State.SUCCESS:
            self.rate.sleep()
